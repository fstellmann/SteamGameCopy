...
namespace SteamGameCopy
{
    ...
    {
        public static void CreateFromDirectory(string sourceDirectoryName, string destinationArchiveFileName, IProgress<double> progress, CancellationToken cts)
        {
            ...
            using (ZipArchive archive = ZipFile.Open(destinationArchiveFileName, ZipArchiveMode.Create))
            {
                foreach (FileInfo file in sourceFiles)
                {
                    ...
                    ZipArchiveEntry entry = archive.CreateEntry(Path.GetFullPath(sourceDirectoryName).TrimEnd(Path.DirectorySeparatorChar).Split(Path.DirectorySeparatorChar).Last()+@"\"+entryName, CompressionLevel.Fastest);
